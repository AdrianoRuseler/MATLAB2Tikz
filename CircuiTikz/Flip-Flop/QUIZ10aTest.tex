...
\node[and gate US, draw, logic gate inputs=nn] at ($(B)+(1.5,-2)$) (gate1) {1};
\node[xor gate US, draw, logic gate inputs=nn] at ($(gate1)+(0,-1)$) (gate2) {2};
\node[nor gate US, draw, logic gate inputs=nn, anchor=input 1] at ($(gate1.output)+(1,-0.5)$) (gate3) {3};
\draw (A) |- (gate1.input 1);
\draw (NotB) |- (gate1.input 2);
\draw (NotA) |- (gate2.input 1);
\draw (B) |- (gate2.input 2);

\draw (4.5,-0.8) node[flipflop JK, rotate=90, scale=0.9, flipflop def={t2={\texttt{CLK}}}](ff1){};

\draw (gate1.output) -- ([xshift=0.5cm]gate1.output) node[above] {} |- (gate3.input 1);
...
\draw (gate3.output) --  ([xshift=0.2cm]gate3.output) node[above] {} |- (ff1.pin 2);
\draw (gate2.output)  -- ([xshift=2.5cm]gate2.output) node[above] {} |- (ff1.pin 3);	
\draw (gate1.output)  to[short, -*] ([xshift=0.5cm]gate1.output) node[above] {} |- (ff1.pin 1);
\draw (gate2.output)  to[short, -*] ([xshift=0.5cm]gate2.output) node[above] {};

%\draw (gate2.output)  to[short, -*] ([xshift=3cm]gate2.output) node[above] {} |- (ff1.pin 3);
